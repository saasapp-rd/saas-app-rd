...
const MENU = [
  ...
  { icon: "🚨", title: "Active Incidents",          desc: "All open incidents",                   href: "/coordinator" },
      ...
      <main className="flex-1 flex flex-col px-5 py-5 gap-3 max-w-lg mx-auto w-full">
        ...
        {MENU.map((item) => (
          <Link
            key={item.title}
            href={item.href}
            className="flex items-center gap-4 px-4 py-4 rounded-xl border transition-all hover:border-red-300"
