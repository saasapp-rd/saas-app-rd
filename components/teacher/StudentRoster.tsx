...
type Status = "none" | "reported" | "with_me" | "loading" | "error"
...
export default function StudentRoster({
  ...
  async function reportMissing(studentId: string) {
    ...
    try {
      ...
      if (res.ok) {
        setStatus(studentId, data.duplicate ? "reported" : "reported")
      } else {
        setStatus(studentId, "error")
        setError(studentId, data.error ?? "Failed to report")
      }
    } catch {
      setStatus(studentId, "error")
      setError(studentId, "Network error — try again")
    }
  }
  ...
  async function withMe(studentId: string) {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setStatus(studentId, "with_me")
      } else {
        setStatus(studentId, "error")
        setError(studentId, data.error ?? "Failed")
      }
    } catch {
      setStatus(studentId, "error")
      setError(studentId, "Network error — try again")
      ...
      {students.map(s => {
        ...
        return (
          <div key={s.id} className="rounded-xl px-4 py-2.5 border"
            ...
            {errMsg && (
              <p className="text-[10px] mt-1 font-semibold" style={{ color: "#CE2033" }}>
                {errMsg}
              </p>
            )}
          </div>
